  ...
  return (
    <div className="relative h-[400px] overflow-hidden shadow-xl bg-[#020202]">
      {slides.map((s, i) => (
        ...
        >
          <div
            ...
            style={{ backgroundImage: `url(${s.img})`, filter: 'brightness(.48) contrast(.96)' }}
          />
      ...
      <button
        onClick={prev}
        aria-label="Prev"
        className="hidden md:flex absolute left-4 top-1/2 -translate-y-1/2 items-center justify-center w-10 h-10 rounded-full bg-white/60 z-30"
      >
      ...
      <button
        onClick={next}
        aria-label="Next"
        className="hidden md:flex absolute right-4 top-1/2 -translate-y-1/2 items-center justify-center w-10 h-10 rounded-full bg-white/60 z-30"
      >
      ...
      <div className="absolute left-1/2 -translate-x-1/2 bottom-6 flex items-center gap-2 z-30">
        {slides.map((_, i) => (
          <button
            ...
            className={`w-3 h-3 rounded-full ${i === index ? 'bg-[#E4D161] scale-110' : 'bg-white/60'}`}
          />
        ))}
      </div>
    </div>
